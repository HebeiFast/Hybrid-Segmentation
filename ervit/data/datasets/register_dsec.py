...
def _get_dsec_meta():
    stuff_classes = [k["readable"] for k in DSEC_SEM_SEG_CATEGORIES if k["evaluate"]]
    assert len(stuff_classes) == 11

    stuff_colors = [k["color"] for k in DSEC_SEM_SEG_CATEGORIES if k["evaluate"]]
    assert len(stuff_colors) == 11
...
def register_all_dsec(root):
    ...
    root = os.path.join(root, "DSEC_Semantic_HD")
    meta = _get_dsec_meta()
